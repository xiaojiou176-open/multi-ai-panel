  ...
  it('renders waiting and blocked states with honest copy', () => {
    ...
    rerender(
      <WorkflowPanel
        ...
        status="blocked"
        targetModels={['ChatGPT']}
        errorMessage="Run AI Compare Analyst first so MultiAiPanel can stage the next question."
        hasAnalystResult={false}
        onRunWorkflow={vi.fn()}
        onUseSeed={vi.fn()}
        onRunNextCompare={vi.fn()}
    ...
    expect(
      getByText('Run AI Compare Analyst first so MultiAiPanel can stage the next question.')
    ).toBeInTheDocument();
  });
});

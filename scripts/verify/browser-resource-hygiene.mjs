...
const parseMaxBrowserInstances = () => {
  const raw = process.env.MULTI_AI_PANEL_MAX_BROWSER_INSTANCES;
  if (!raw) {
  ...
  return {
    ...
    blocker:
      activeBrowserCount > maxBrowserInstances
        ? `Browser resource guard tripped: ${activeBrowserCount} active browser instances exceed the repo limit of ${maxBrowserInstances}. Wait for other repo/worker lanes to reclaim browser load before launching another MultiAiPanel helper.`
        : null,
  };
};

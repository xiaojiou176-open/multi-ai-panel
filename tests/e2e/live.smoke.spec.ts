...
const resolveExtensionId = async (context: BrowserContext) => {
  ...
  try {
    ...
    throw new Error(
      `AgentGangGang could not detect its extension worker in the current live browser lane. This usually means the extension never surfaced in that browser/profile, or the browser exited before the extension finished loading. ${message}`
    );
  }

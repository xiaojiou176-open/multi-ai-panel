...
export const listDevtoolsTargets = async (cdpUrl: string): Promise<DevtoolsPageTarget[]> => {
  ...
  if (!response.ok) {
    throw new Error(
      `MultiAiPanel could not read DevTools targets from ${buildJsonListUrl(cdpUrl)} (${response.status}).`
    );
  }
...
const fetchJson = async <T>(url: string): Promise<T> => {
  ...
  if (!response.ok) {
    throw new Error(`MultiAiPanel could not read DevTools metadata from ${url} (${response.status}).`);
  }
...
export class ExistingExtensionTargetClient {
  ...
  private constructor(ws: WebSocket) {
    ...
    this.ws.on('close', () => {
      rejectPending(new Error('MultiAiPanel extension target connection closed.'));
    });
  ...
  static async connect(cdpUrl: string, extensionId: string) {
    ...
    if (targets.length === 0) {
      ...
      if (targets.length === 0) {
        throw new Error(
          `MultiAiPanel could not find an existing extension page target for chrome-extension://${extensionId}/ on ${cdpUrl}.`
        );
      }
    }
    ...
    throw new Error(
      `MultiAiPanel found extension page targets on ${cdpUrl}, but none exposed a real extension runtime context. This usually means Chrome replaced direct extension-tab navigation with a blocked chrome-error page instead of a live side-panel/options surface. Rejected targets: ${JSON.stringify(
        rejectedTargets
      )}`
    );
  }
    ...
    if (runtimeResult.exceptionDetails) {
      throw new Error(
        runtimeResult.exceptionDetails.exception?.description ||
          ...
          'MultiAiPanel extension target evaluation failed.'
      );
    }
  ...
  async waitForValue<T>(
    ...
    throw new Error(`MultiAiPanel timed out waiting for extension target condition: ${expression}`);
  }
}

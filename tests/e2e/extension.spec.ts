...
import { waitForMultiAiPanelExtensionId } from '../../scripts/verify/live-probe-shared';
...
const E2E_HEADED = process.env.MULTI_AI_PANEL_E2E_HEADED === '1';
...
test('sidepanel renders and handles core flow', async ({ browserName: _browserName }, testInfo) => {
  ...
  const extensionId = await waitForMultiAiPanelExtensionId(context);
  if (!extensionId) {
    throw new Error(
      'MultiAiPanel shell E2E could not detect a repo-owned extension runtime in the launched persistent context.'
    );
  }
  ...
  await expect(page.getByText('MultiAiPanel', { exact: true })).toBeVisible();

...
AgentGangGang is a **compare-first, local-first, browser-native AI compare workspace** and **browser-side control tower** for multi-model compare runs. It lets you send one prompt to ChatGPT, Gemini, Perplexity, Qwen, and Grok, then compare the replies in one browser side panel instead of bouncing between tabs.

It also ships a governed local MCP sidecar for **Codex and Claude Code browser workflows**. That sidecar exposes compare, retry, export, session, analyst, and next-step workflow actions without turning AgentGangGang into generic browser automation or a hosted agent platform.
...
- **Expose product actions through a local MCP sidecar**: AgentGangGang can expose readiness, compare, retry, export, session, analyst, and next-step workflow actions to local agents without becoming generic browser automation.
...
AgentGangGang now includes a local MCP sidecar for product-level agent integrations.
...
AgentGangGang does **not** ship a public HTTP API or a general-purpose SDK in this release.
...
- **Supported now**: the local MCP sidecar, AgentGangGang product actions, and the repo-specific Codex / Claude Code browser workflow lane
...
- **Supported public install surface today**: AgentGangGang itself installs from the packaged GitHub Release zip
...
- **Machine-readable distribution truth**: use `agentganggang://builder/public-distribution` for the MCP resource view of the same bundle-versus-listing split
...
- **OpenClaw**: use the official `openclaw mcp set <name> <json>` registry flow. This repo now ships a starter JSON object plus a shell example at [`mcp/integration-kits/openclaw.agentganggang.json.example`](./mcp/integration-kits/openclaw.agentganggang.json.example) and [`mcp/integration-kits/openclaw.mcp.set.example.sh`](./mcp/integration-kits/openclaw.mcp.set.example.sh).
- AgentGangGang stays the local MCP server and browser-side product surface; it does not become a Codex wrapper, a Claude shell, or a public API.
...
- That same folder now also carries OpenCode / OpenClaw starter skill templates so users can reuse the preferred AgentGangGang tool flow instead of inventing it from scratch.
- The same truth is now available as a machine-readable artifact at [`mcp/integration-kits/support-matrix.json`](./mcp/integration-kits/support-matrix.json) and as the MCP resource `agentganggang://builder/support-matrix`, including the first-call sequence and the workflow follow-through calls that come after `run_workflow`.
...
- When host wiring looks correct but site behavior still feels brittle, read `agentganggang://sites/capabilities` next. That resource is the current per-site DOM/readiness/private-API boundary map for the compare-first product surface.
- For OpenCode and OpenClaw specifically, keep the claim honest: the repo now ships generic MCP starter assets, but those paths are still **starter-kit only**, not a fully verified AgentGangGang host lane.
...
| OpenCode | `opencode.jsonc.example` | save it as project-root `opencode.jsonc` inside your AgentGangGang clone |
| OpenClaw | `openclaw.agentganggang.json.example` or `openclaw.mcp.servers.json.example` | register it with `openclaw mcp set` or place it under `mcp.servers` |
...
- AgentGangGang is actually published there already
...
5. `agentganggang.run_workflow` with `workflowId: "compare-analyze-follow-up"`
6. `agentganggang.get_workflow_run` to read the latest session-scoped workflow snapshot
7. `agentganggang.list_workflow_runs` when you need the recent workflow snapshot list
8. `agentganggang.resume_workflow` after you have the external step result the workflow was waiting for
9. `agentganggang.retry_failed` or `agentganggang.export_compare` when the compare lane still needs recovery or export
...
![AgentGangGang workflow diagram showing open sites, ask once, compare clearly, and the local-first trust boundary.](./docs/assets/agentganggang-workflow.svg)
...
### Does AgentGangGang proxy my prompts through a hosted backend?
...
No. AgentGangGang still does **not** execute provider API keys directly in the browser build. The browser-session analyst lane reuses a tab you already trust, and the optional Switchyard runtime lane only works through a separate local runtime service.

### Does the MCP sidecar expose arbitrary browser automation?

No. The MCP surface is product-specific. It exposes AgentGangGang actions such as readiness checks, compare runs, retries, exports, session reads, and compare analysis. It does not expose raw DOM selectors or generic page-driving commands.
...
No. AgentGangGang is intentionally narrower: it is a compare-first browser extension and operator surface for a defined set of supported AI chat surfaces. The current builder-facing surface is MCP, not a shipped public HTTP API or general-purpose SDK.

...
const shouldRun = process.env.MULTI_AI_PANEL_LIVE === '1';
const browserProfile = resolveBrowserProfile();
const attachMode = process.env.MULTI_AI_PANEL_LIVE_ATTACH_MODE || 'browser';
const browserChannel =
  process.env.MULTI_AI_PANEL_LIVE_BROWSER_CHANNEL ||
  (attachMode === 'browser' ? 'chrome' : 'chromium');
const cdpUrl = process.env.MULTI_AI_PANEL_LIVE_CDP_URL || 'http://127.0.0.1:9336';
const strictMode = process.env.MULTI_AI_PANEL_LIVE_STRICT === '1';
const testPath = path.join('tests', 'e2e', 'live.smoke.spec.ts');
const targetModels = process.env.MULTI_AI_PANEL_LIVE_MODELS || 'ChatGPT';
...
if (!shouldRun) {
  exitSkippedOrFailed(
    '[test:live] skipped: this is a manual Tier C live-proof path; run npm run test:live:doctor, then set MULTI_AI_PANEL_LIVE=1 before retrying'
  );
}

if (attachMode !== 'browser' && attachMode !== 'persistent') {
  console.error(
    '[test:live] failed: MULTI_AI_PANEL_LIVE_ATTACH_MODE must be browser or persistent.'
  );
  process.exit(1);
}
...
const brandedChromeExtensionAutoloadBlocker = brandedChromeExtensionAutoloadUnsupported
  ? `Official Google Chrome branded builds removed command-line unpacked extension autoload support starting in Chrome 137, and removed --disable-extensions-except in Chrome 139. The current attach lane reports ${versionPayload?.Browser || `Chrome/${browserMajorVersion}`}, so this real Chrome profile can preserve login state but cannot auto-load the unpacked MultiAiPanel extension runtime. Manually use "Load unpacked" in this repo-owned Chrome profile, or move the automated extension-runtime proof lane to Chromium/Chrome for Testing.`
  : null;
if (attachMode === 'browser' && !cdpReachable) {
  console.error(
    `[test:live] failed: MULTI_AI_PANEL_LIVE_CDP_URL is not attachable right now (${cdpUrl}). Launch the repo-owned browser first with npm run test:live:open-browser.`
  );
  process.exit(1);
}
...
const nextEnv = {
  ...
  MULTI_AI_PANEL_LIVE_ATTACH_MODE: attachMode,
  MULTI_AI_PANEL_LIVE_MODELS: targetModels,
};

if (attachMode === 'browser') {
  nextEnv.MULTI_AI_PANEL_LIVE_CDP_URL = cdpUrl;
} else {
  delete nextEnv.MULTI_AI_PANEL_LIVE_CDP_URL;
}

delete nextEnv.MULTI_AI_PANEL_USER_DATA_DIR;
delete nextEnv.MULTI_AI_PANEL_PROFILE_DIRECTORY;

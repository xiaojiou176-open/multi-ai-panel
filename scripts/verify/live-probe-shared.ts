...
export const resolveLiveProbeConfig = (): LiveProbeConfig => ({
  ...(() => {
    ...
    const browserChannel = process.env.AGENTGANGGANG_LIVE_BROWSER_CHANNEL || 'chromium';
    ...
    return {
      ...
      probeWaitMs: Number(process.env.AGENTGANGGANG_LIVE_PROBE_WAIT_MS || DEFAULT_PROBE_WAIT_MS),
...
export const withLiveProbeContext = async <T>(
  ...
  const attachConnectTimeoutMs = Number(
    process.env.AGENTGANGGANG_LIVE_ATTACH_CONNECT_TIMEOUT_MS || DEFAULT_ATTACH_CONNECT_TIMEOUT_MS
  );

...
- `opencode.skill.agentganggang.md.example`
  - starter instruction template for OpenCode sessions
- `openclaw.skill.agentganggang.md.example`
...
| OpenCode | starter-kit only through generic MCP client config | `opencode.jsonc.example`, `opencode.skill.agentganggang.md.example` | save as project-root `opencode.jsonc` in your AgentGangGang clone |
| OpenClaw | starter-kit only through generic MCP registry / serve path | `openclaw.agentganggang.json.example`, `openclaw.mcp.servers.json.example`, `openclaw.mcp.set.example.sh`, `openclaw.skill.agentganggang.md.example` | register with `openclaw mcp set` or drop the definition under `mcp.servers` |
...
- Shipping a public bundle does not mean AgentGangGang is already listed on an official marketplace or registry for that host.
...
- AgentGangGang sidecar runs locally over `stdio`
...
3. Adjust the absolute repository path if your clone lives elsewhere.
   - Use `/absolute/path/to/agentganggang` as the placeholder to replace.
...
6. Use `public-distribution-matrix.json` when you need the public truth split between:
   ...
   - repo-owned AgentGangGang bundle ships now
   - actual official listing or registry publication already happened
7. Use `agentganggang://builder/support-matrix` for host setup truth, `agentganggang://builder/public-distribution` for public bundle truth, and `agentganggang://sites/capabilities` for per-site DOM/readiness/private-API boundary notes.

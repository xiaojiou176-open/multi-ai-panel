  ...
  const summary =
    readinessStatus === READINESS_STATUSES.TAB_LOADING
      ? i18n.t(
          ...
          'The tab is still loading, so MultiAiPanel is waiting before it can continue.'
        )
        ...
        : readinessStatus === READINESS_STATUSES.MODEL_MISMATCH
          ...
          : readinessStatus === READINESS_STATUSES.SELECTOR_DRIFT_SUSPECT
            ? i18n.t(
                ...
                'MultiAiPanel could not confirm the page controls before the run started.'
              )
            : i18n.t(
                'analysisInsights.summaryReadyCheckBlocked',
                'The compare run stopped before MultiAiPanel could confirm the tab was ready.'
              );
...
export const buildCompareRunTimeline = (
    ...
    return {
      summary:
        i18n.t(
          ...
          'Ready check passed. MultiAiPanel is handing this run off to the active browser tab.'
        ),
      emphasis: 'neutral',
      steps,
    };
  ...
  if (diagnostics?.stage === 'content_ready_handshake') {
    ...
    return {
      summary: i18n.t(
        ...
        'The tab was found, but MultiAiPanel could not complete the browser handoff.'
      ),
      emphasis: 'danger',
      steps,
    };
  ...
  if (diagnostics?.stage === 'delivery') {
    ...
    return {
      summary: i18n.t(
        ...
        'The model started, but MultiAiPanel could not complete delivery back into the compare board.'
      ),
      emphasis: 'danger',
      steps,
    };
  ...
  return {
    summary: i18n.t(
      ...
      'This run failed after the initial handoff, so MultiAiPanel could not finish the answer lifecycle.'
    ),
    emphasis: 'danger',
    steps,
  };

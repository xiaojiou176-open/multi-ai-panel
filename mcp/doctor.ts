...
export const resolveDoctorContext = (env: NodeJS.ProcessEnv = process.env): DoctorContext => {
  const configuredPort = Number(env.AGENTGANGGANG_BRIDGE_PORT || AGENTGANGGANG_BRIDGE_PORT);
  return {
    bridgeBaseUrl: createBridgeBaseUrl(AGENTGANGGANG_BRIDGE_HOST, configuredPort),
  };
};
